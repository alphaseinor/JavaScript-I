...
//let lastCar = 0;
console.log(`${inventory[length].car_year}  ${inventory[length].car_make}  ${inventory[length].car_model}`);

// ==== Challenge 3 ====
// The marketing team wants the car models listed alphabetically on the website. Sort all the car model names into alphabetical order and log the results in the console
let carModels = inventory.map(eachElement => eachElement.car_make)
...
let carYears = inventory.map(eachElement => eachElement.car_year);
console.log(carYears);

// ==== Challenge 5 ====
// The car lot manager needs to find out how many cars are older than the year 2000. Using the carYears array you just created, find out how many cars were made before the year 2000 by populating the array oldCars and logging it's length.
let oldCars = inventory.filter(x =>  x.car_year < 2000);

console.log(oldCars.length);

// ==== Challenge 6 ====
// A buyer is interested in seeing only BMW and Audi cars within the inventory.  Return an array that only contains BMW and Audi cars.  Once you have populated the BMWAndAudi array, use JSON.stringify() to show the results of the array in the console.
let BMWAndAudi = inventory.filter(x =>  x.car_make === "Audi" || x.car_make === "BMW");
